...
class Store with ChangeNotifier {
  late List<String> _users; // list of users
  Map<String, List<Message>>? _messages; // messages of joined user
  late int _joinStatus; // join status
  late int _loggedStatus; // user joins second time using login . login status
  late TextEditingController _searchPattern; // search in userslist
  late TextEditingController _curMessage; // cur message user is typing
  ...
  sendMessage(String to) async {
    try {
      final Response response =
          await api.sendmessage(_user!.text, to, _curMessage.text);
